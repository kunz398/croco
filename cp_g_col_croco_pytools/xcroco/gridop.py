    ...
    try:
        ...
        if 'Cs_r' not in ds:
            if io.find_var(model,'Cs_r',ds,gd) is not None:
                ...
                ds['Cs_r'] = ds['Cs_rho']
            elif io.find_var(model,'Cs_rho',ds,gd) is not None:
                ds['Cs_r'] = io.find_var(model,'Cs_rho',ds,gd)
            else:
...
def order_dims(var):
    ...
    return var.cf.transpose(
        *[
            ...
            dim
            for dim in ["t", "s", "s_w", "z", "z_r", "z_u", "z_v", "y", "y_v", "x", "x_u"]
            if dim in var.dims
        ]
    )
...
def get_z(model, ds=None, z_sfc=None, h=None, xgrid=None, vgrid='r',
    ...
    if vgrid in ['r', 'rho']:
        sc = ds['sc_r'] if 'sc_r' in ds else ds['sc_rho']
        sc = ds['sc_r'] if 'sc_r' in ds else ds['sc_rho'] if 'sc_r' in ds else ds['sc_rho']
