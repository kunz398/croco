...
class CROCOForecastRunner:
    ...
    def __init__(self, config_file=None):
        ...
        try:
            from cp_g_col_croco_pytools.Forecast_CROCO.old__croco_tools_params import add_tides_fcst, grdname
    ...
    def setup_directories(self):
        ...
        try:
            from cp_g_col_croco_pytools.Forecast_CROCO.old__croco_tools_params import RUN_dir
            self.RUNDIR = RUN_dir
        except ImportError:
    ...
    def preprocess_forcing(self):
        ...
        try:
            ...
            try:
                import cp_g_col_croco_pytools.Forecast_CROCO.old__croco_tools_params as old__croco_tools_params
            ...
            if run_gfs:
                ...
                try:
                    proc = subprocess.Popen([sys.executable, '-u', 'make_gfs.py'],
            ...
            if run_ogcm:
                ...
                try:
                    proc = subprocess.Popen([sys.executable, '-u', 'make_OGCM_frcst.py'],
                                            stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                            text=True, bufsize=1)
                    for line in proc.stdout:
    ...
    def generate_tidal_forcing(self):
        ...
        try:
            # Resolve PREPRO path dynamically (env override -> local repo -> legacy path)
            script_dir = Path(__file__).resolve().parent
            candidate_prepro_paths = []

            env_prepro_path = os.environ.get('CROCO_PREPRO_PATH')
            if env_prepro_path:
                candidate_prepro_paths.append(Path(env_prepro_path).expanduser().resolve())

            candidate_prepro_paths.extend([
                (script_dir / '..' / 'prepro').resolve(),
                (script_dir / '..' / 'PREPRO').resolve(),
                Path('/DATA/CROCO/croco_pytools-v1.0.3/prepro')
            ])

            prepro_path = None
            make_tides_script = None
            for candidate in candidate_prepro_paths:
                candidate_script = candidate / 'make_tides.py'
                if candidate_script.exists():
                    prepro_path = str(candidate)
                    make_tides_script = str(candidate_script)
                    break

            if not make_tides_script:
                checked_paths = ', '.join(str(p) for p in candidate_prepro_paths)
                logger.error(f"make_tides.py not found. Checked PREPRO paths: {checked_paths}")
                return False

            logger.info(f"Using PREPRO path: {prepro_path}")
...
input_dir = '{os.path.join(prepro_path, "..", "DATASETS_CROCOTOOLS", "TPXO7") + os.sep}'
input_file = 'TPXO7.nc'
input_type = 'Re_Im'
multi_files = False
...
'''
            ...
            try:
                ...
                proc = subprocess.Popen([
                    sys.executable, '-u', tides_config_path
                ], stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                   text=True, bufsize=1, cwd=prepro_path)
                for line in proc.stdout:
                    logger.info(f"[make_tides] {line.rstrip()}")
                proc.wait()
                ...
                if proc.returncode == 0:
                    ...
                    logger.error(f"Error generating tidal forcing (exit code {proc.returncode})")
                    return False
