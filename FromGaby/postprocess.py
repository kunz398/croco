...
if __name__ == '__main__':
    ...
    for input_file in input_files:
        ...
        # zeta is now added inside croco_sigma_to_z with correct dims and CF attributes
    ...
    if len(output_files) <= 1:
        print(f"Only one output file; renaming to {os.path.basename(concat_output)}")
        os.rename(output_files[0], concat_output)
    else:
        print(f"Concatenating {len(output_files)} files -> {os.path.basename(concat_output)}")
        # Make time a record dimension in each file so ncrcat can concatenate
        rec_files = []
        for f in output_files:
            rec = f.replace('.nc', '_rec.nc')
            subprocess.run(['ncks', '-O', '--mk_rec_dmn', 'time', f, rec], check=True)
