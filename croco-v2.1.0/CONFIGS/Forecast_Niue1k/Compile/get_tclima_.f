     ...
     &             uclm_id,  vclm_id
      real cff
      integer*4 i,itrc, lstr,lvar,lenstr, ierr, nf_fread, advance_cycle
      integer*4 nf_byte
      integer*4 nf_int1
      integer*4 nf_char
      integer*4 nf_short
     ...
     &         tclm_cycle(itrc), tclm_ncycle(itrc), tclm_rec(itrc))
            if (may_day_flag.ne.0) return
            ittclm(itrc)=2
            tclm_time(1,itrc)=-1.D+20
            tclm_time(2,itrc)=-1.D+20
          endif
        enddo
      endif
      do itrc=1,NT
 ...
 10     i=3-ittclm(itrc)
     ...
     &                                *tclm_ncycle(itrc)
         if (tclm_time(ittclm(itrc),itrc).eq.-1.D+20)
     &       tclm_time(ittclm(itrc),itrc)=tclm_time(i,itrc)
         ierr=nf_fread (tclima(0,0,1,i,itrc),
  ...
  99  may_day_flag=2
     ...
     &             uclm_tid, ubclm_id,    vbclm_id,
     &             uclm_id,  vclm_id
      integer*4 Istr,Iend,Jstr,Jend, itrc,i,j,k, it1,it2
      real    cff, cff1, cff2
      ...
      do itrc=1,NT
       if (got_tclm(itrc)) then
        ...
        if (tclm_cycle(itrc).lt.0.D0) then
          if (iic.eq.0) then
