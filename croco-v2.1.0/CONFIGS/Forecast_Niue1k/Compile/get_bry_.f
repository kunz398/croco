     ...
     &    tbry_north_dt(0:Lm+1+padd_X,N,2,NT)
      ...
      real cff
      character*180 units
      integer*4  lstr,lvar,lenstr, latt, lorig, ierr, ierr_all, itrc
  ...
  1   format(' ERROR in get_all_bry: cannot find variable ''',A,'''')
     ...
     &            r3dvar)
            ...
            ierr=ierr_all
            if (ierr.eq.0) then
               write(stdout,'(6x,A,9x,A,1x,F10.4,1x,I4)')
