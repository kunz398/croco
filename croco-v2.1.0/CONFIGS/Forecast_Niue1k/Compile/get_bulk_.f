     ...
     &                  bulk_cycle, bulk_ncycle, bulk_rec)
        ...
        stf_scale(isalt)=0.01D0/86400.D0
      endif
  1   i=3-itbulk
       cff=time+0.5D0*dt
     ...
     &            return
        ...
        if (ierr .ne. 0) goto 7
        ierr=nf_get_var1_double (ncidbulk, bulk_tid, bulk_rec, cff)
        if (ierr .ne. nf_noerr) then
          write(stdout,6) 'bulk_time', bulk_rec
          goto 99
        endif
        bulk_time(i)=cff*day2sec+bulk_cycle*bulk_ncycle
        if (bulk_time(itbulk).eq.-1.D+20)
     &          bulk_time(itbulk)=bulk_time(i)
        if (ltairgrd.eq.1) then
          ierr=nf_fread (tairg(0,0,i), ncidbulk, tair_id,
     &                                            bulk_rec,  r2dvar)
        else
          ierr=nf_get_var1_double (ncidbulk,tair_id,bulk_rec,tairp(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (lrhumgrd.eq.1) then
          ierr=nf_fread (rhumg(0,0,i), ncidbulk, rhum_id,
     &                                            bulk_rec,  r2dvar)
        else
          ierr=nf_get_var1_double (ncidbulk,rhum_id,bulk_rec,rhump(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (lradlwgrd.eq.1) then
          ierr=nf_fread (radlwg(0,0,i), ncidbulk, radlw_id,
     &                                            bulk_rec,  r2dvar)
        else
          ierr=nf_get_var1_double (ncidbulk,radlw_id,bulk_rec,radlwp(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (lradswgrd.eq.1) then
          ierr=nf_fread (radswg(0,0,i), ncidbulk, radsw_id,
     &                                            bulk_rec,  r2dvar)
        else
          ierr=nf_get_var1_double (ncidbulk,radsw_id,bulk_rec,radswp(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (lprategrd.eq.1) then
          ierr=nf_fread (prateg(0,0,i), ncidbulk, prate_id,
     &                                            bulk_rec,  r2dvar)
        else
          ierr=nf_get_var1_double (ncidbulk,prate_id,bulk_rec,pratep(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (luwndgrd.eq.1) then
          ierr=nf_fread(uwndg(0,0,i), ncidbulk, uwnd_id,
     &                                              bulk_rec, u2dvar)
        else
          ierr=nf_get_var1_double(ncidbulk,uwnd_id,bulk_rec,uwndp(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (lvwndgrd.eq.1) then
          ierr=nf_fread(vwndg(0,0,i), ncidbulk, vwnd_id,
     &                                              bulk_rec, v2dvar)
        else
          ierr=nf_get_var1_double(ncidbulk,vwnd_id,bulk_rec,vwndp(i))
        endif
        if (ierr .ne. nf_noerr) then
       ...
       itbulk=i
        write(stdout,'(6x,A,1x,A,1x,F10.4,1x,I4)')
     &              'GET_BULK   --',
  ...
  99  may_day_flag=2
     ...
     &           jul_off=2440000.D0)
      ...
      common /Z0B_VAR/zob
      integer*4 Istr,Iend,Jstr,Jend, i,j, k, it1,it2
      integer*4 imax,jmax
          ...
          if (ltairgrd.eq.1) then
            ...
            do j=JstrR,min(JendR,Mm)
              do i=IstrR,min(IendR,Lm)
                wspd(i,j)=sqrt(0.25D0*(uwnd(i,j)+uwnd(i+1,j))**2+
     &                         0.25D0*(vwnd(i,j)+vwnd(i,j+1))**2)
              enddo
            enddo
            if (istr.eq.1) then
              do j=Jstr,Jend
            ...
            do j=JstrR,JendR
              do i=IstrR,IendR
                ...
                uwnd(i,j)=val6
                vwnd(i,j)=val7
                wspd(i,j) = SQRT(val6**2.0D0+val7**2.0D0)
              enddo
            enddo
          endif
        endif
...
!$acc update device( wspd )
          ...
          do j=JstrR,JendR
            do i=IstrR,IendR
              ...
              uwnd(i,j)=val6
              vwnd(i,j)=val7
              wspd(i,j)=sqrt(uwnd(i,j)**2.0D0+vwnd(i,j)**2.0D0)
            enddo
          enddo
        endif
!$acc update device( tair, rhum
!$acc&  ,radlw ,radsw
!$acc&  ,srflx
!$acc&  ,prate
!$acc&  ,uwnd,vwnd
