  ...
  1   i=3-itssh
       cff=time+0.5D0*dt
        ...
        if (ierr.ne.0) goto 7
        ierr=nf_get_var1_double(ncidclm, ssh_tid, ssh_rec, cff)
        if (ierr.ne.NF_NOERR) then
          write(stdout,6) 'ssh_time', ssh_rec
          goto 99
        endif
        ssh_time(i)=cff*day2sec+ssh_cycle*ssh_ncycle
        if (ssh_time(itssh).eq.-1.D+20) ssh_time(itssh)=ssh_time(i)
        ierr=nf_fread(sshg(0,0,i), ncidclm, ssh_id,
     &                                          ssh_rec, r2dvar)
        if (ierr.ne.NF_NOERR) then
